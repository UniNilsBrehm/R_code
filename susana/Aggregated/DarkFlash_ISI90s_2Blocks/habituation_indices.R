...
# ==============================================================================
FIRST_WIN_END  <- 2    # stimuli 1:FIRST_WIN_END   = "first" window
LAST_WIN_START <- 59   # stimuli LAST_WIN_START:60  = "last"  window
...
# Compute HI_between using a specific window column (e.g. mean_, first_, center_, last_)
# For prob/peak/cumsum: Block1 > Block2 = retention => flip sign (* -1)
# For delay: Block2 > Block1 = retention => no flip
hi_between <- function(b1, b2, flip = TRUE) {
  r <- safe_ratio(b1, b2)
  if (flip) r * -1 else r
}
...
df_indices_delay <- df_block_sub %>%
  ...
  mutate(
    # Delay: Block2 > Block1 = retention => no flip (hi_between flip = FALSE)
    HI_between_mean   = hi_between(mean_delay_Block2,   mean_delay_Block1,   flip = FALSE),
    HI_between_first  = hi_between(first_delay_Block2,  first_delay_Block1,  flip = FALSE),
    HI_between_center = hi_between(center_delay_Block2, center_delay_Block1, flip = FALSE),
    HI_between_last   = hi_between(last_delay_Block2,   last_delay_Block1,   flip = FALSE),
    # Delay SI: last > first = habituation => positive
    SI_block1         = safe_ratio(last_delay_Block1,  first_delay_Block1),
    SI_block2         = safe_ratio(last_delay_Block2,  first_delay_Block2),
    # Absolute "drop" for delay = increase (last - first)
    abs_drop_b1       = last_delay_Block1  - first_delay_Block1,
    abs_drop_b2       = last_delay_Block2  - first_delay_Block2,
    prop_drop_b1      = case_when(
      is.na(first_delay_Block1) | first_delay_Block1 == 0 ~ NA_real_,
      TRUE ~ (last_delay_Block1 - first_delay_Block1) / first_delay_Block1
    ),
    prop_drop_b2      = case_when(
      is.na(first_delay_Block2) | first_delay_Block2 == 0 ~ NA_real_,
      TRUE ~ (last_delay_Block2 - first_delay_Block2) / first_delay_Block2
    ),
...
run_nonparam_tests <- function(df, index_var, label = "") {
  ...
  kw   <- df_clean %>% kruskal_test(as.formula(paste(index_var, "~ Genotype")))
  dunn <- df_clean %>%
    dunn_test(as.formula(paste(index_var, "~ Genotype")), p.adjust.method = "BH") %>%
    add_significance()
...
p_hi_combined <-
  ...
  plot_annotation(
    ...
    subtitle = paste0(
      "HI = (B1 - B2) / (B1 + B2)  |  positive = retention  |  ",
      "first = stim 1–", FIRST_WIN_END,
      "  |  center = stim ", CENTER_WIN_START, "–", CENTER_WIN_END,
      "  |  last = stim ", LAST_WIN_START, "–", N_STIM
    ),
    tag_levels = list(c(
      "Mean",   "", "", "",
      "First",  "", "", "",
...
p_si_combined <-
  ...
  plot_annotation(
    ...
    subtitle = paste0(
      ...
      "first = stim 1–", FIRST_WIN_END, "  |  last = stim ", LAST_WIN_START, "–", N_STIM
    ),
...
p_abs <- (plot_row(stats_abs_b1, "abs_drop_b1", "Absolute drop") /
  ...
  plot_annotation(
    title    = "Absolute drop (first window − last window)",
    subtitle = paste0("first = stim 1–", FIRST_WIN_END, "  |  last = stim ", LAST_WIN_START, "–", N_STIM),
    tag_levels = list(c("Block 1", "", "", "", "Block 2", "", "", "")),
...
p_asym <- (plot_row(stats_asym_b1, "asymptote_b1", "Asymptote") /
  ...
  plot_annotation(
    ...
    subtitle = paste0("last = stim ", LAST_WIN_START, "–", N_STIM),
    tag_levels = list(c("Block 1", "", "", "", "Block 2", "", "", "")),
    theme = theme(plot.title = element_text(size = 14, face = "bold"),
                  plot.subtitle = element_text(size = 10),
                  plot.tag = element_text(size = 10, face = "bold", angle = 90),
...
p_init <- (plot_row(stats_init_b1, "initial_b1", "Initial reactivity") /
  ...
  plot_annotation(
    ...
    subtitle = paste0("first = stim 1–", FIRST_WIN_END),
    tag_levels = list(c("Block 1", "", "", "", "Block 2", "", "", "")),
    theme = theme(plot.title = element_text(size = 14, face = "bold"),
                  plot.subtitle = element_text(size = 10),
                  plot.tag = element_text(size = 10, face = "bold", angle = 90),
