...
plot_habituation_response <- function(
    ...
    save_fig_dir  = NULL,
    var_name      = NULL,
    raw_data      = c("aggregate", "trials", "individual"),  # NEW: added "individual"
    trials_alpha        = 0.05,
    trials_size         = 0.4,
    jitter_width        = 0.10,
    jitter_height       = 0,
    individual_alpha    = 0.25,   # NEW
    individual_linewidth = 0.4,   # NEW
    ndraws        = NULL,         # NEW: optional draw subsampling for individual curves
    y_limits      = NULL
) {
  ...
  # ---- posterior fitted values (population-level) ----------------------------
  pred_resp <- fitted(
    fit_model,
    newdata    = new_resp,
    re_formula = NA,
  ...
  if (raw_data == "individual") {
    ...
    # Summarise over posterior draws per animal × stimulus
    pred_per_animal <- new_resp_animals %>%
      mutate(.row = row_number()) %>%
      mutate(fit = colMeans(epred_animals))  # posterior mean per row
  }
